...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Curriculum Item</h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('curriculum.update', $curriculumItem->curriculum_id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('class_subject_id') is-invalid @enderror" id="class_subject_id" name="class_subject_id" required>
                        ...
                        @foreach($classSubjects as $classSubject)
                            <option value="{{ $classSubject->class_subject_id }}" {{ old('class_subject_id', $curriculumItem->class_subject_id) == $classSubject->class_subject_id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $curriculumItem->title) }}" required>
                    @error('title')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="5">{{ old('description', $curriculumItem->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('resource_link') is-invalid @enderror" id="resource_link" name="resource_link" value="{{ old('resource_link', $curriculumItem->resource_link) }}">
                    @error('resource_link')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
